...
\pnum
\effects
Equivalent to \tcode{return *this << s;} where \tcode{s} is an
implementation-defined NTCTS~(\ref{fs.def.ntcts}).
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Appends \tcode{path::preferred_separator} to \tcode{pathstring},
...
Then appends the effective range of \tcode{source} (\ref{path.req})
    or the range \range{first}{last} to \tcode{pathstring},
    converting format and encoding if required (\ref{path.cvt}).
