...
\begin{itemdescr}
\pnum
\effects As if by: \tcode{is \shr x;}
...
\begin{note} Does not apply to \grammarterm{dot} and \grammarterm{dot-dot}. \end{note}
...
\begin{itemize}
\item an added \grammarterm{directory-separator} would be redundant, or
\item an added \grammarterm{directory-separator} would change an relative path to an absolute path, or
\item \tcode{source.empty()} is \tcode{true}, or
\item \tcode{*source.native().cbegin()} is a \grammarterm{directory-separator}.
\end{itemize}
Then appends the effective range of \tcode{source} (\ref{path.req})
...
\begin{itemdescr}
\pnum
\effects \tcode{pathname} (the stored path) is modified as follows:
  \begin{itemize}
...
\begin{itemdescr}
\pnum
\effects Swaps the contents of the two paths \tcode{pathname} and
\tcode{rhs.pathname}.
...
If \grammarterm{root-directory} is composed of \grammarterm{slash} \grammarterm{name}, \grammarterm{slash} is
...
\pnum
\effects
Using \tcode{operator==} to determine if elements match,
determines the first mismatched element of \tcode{*this} and \tcode{base}
using \tcode{mismatch(begin(),} \tcode{end(),} \tcode{base.begin(),} \tcode{base.end())}.

Then:
\begin{itemize}
\item returns \tcode{path()} if
  the first mismatched element of \tcode{*this} is equal to \tcode{begin()} or
  the first mismatched element of \tcode{base} is equal to \tcode{base.begin()}, or
\item returns \tcode{path(".")} if
  the first mismatched element of \tcode{*this} is equal to \tcode{end()} and
  the first mismatched element of \tcode{base} is equal to \tcode{base.end()}, or
\item returns an object of class \tcode{path} composed via
\begin{itemize}
\item application of \tcode{operator/=} (\tcode{path("..")})
  for each element in the half-open range
  [first mismatched element of \tcode{base}, \tcode{base.end()}), and then
\item application of \tcode{operator/=}
  for each element in the half-open range
  [first mismatched element of \tcode{*this}, \tcode{end()}).
\end{itemize}
\end{itemize}
...
\pnum
For the elements of \tcode{pathname} in the generic format,
...
\begin{itemdescr}
\pnum
\effects As if by: \tcode{os << quoted(p.string<charT, traits>());}
...
\tcode{resolve_symlinks} &
  \tcode{0x40000} &  &
  \tcode{permissions()} shall resolve~(\ref{fs.def.pathres} symlinks \\ \rowsep
\end{floattable}
...
\begin{itemdescr}
\pnum
Let \tcode{s} be the \tcode{file_status}, determined
as if by \tcode{status(p, ec)}.

\pnum
\effects Determines \tcode{s}.
...
\tcode{remove_perms} &
  \tcode{status(p).permissions() \& \~(prms \& perms::mask)} \\
\end{floattable}
...
\end{codeblock}
  except \tcode{path()} at the first error occurrence, if any.
...
\item If the attributes indicate a regular file, as if by POSIX \tcode{S_ISREG},
      returns \tcode{file_status(file_type::regular)}. \begin{note}
...
\item Otherwise, if the attributes indicate a directory, as if by POSIX
      \tcode{S_ISDIR}, returns \tcode{file_status(file_type::directory)}.
...
\item Otherwise, if the attributes indicate a block special file, as if by
      POSIX \tcode{S_ISBLK}, returns \tcode{file_status(file_type::block)}.
\item Otherwise, if the attributes indicate a character special file, as if
      by POSIX \tcode{S_ISCHR}, returns \tcode{file_status(file_type::character)}.
\item Otherwise, if the attributes indicate a fifo or pipe file, as if by
      POSIX \tcode{S_ISFIFO}, returns \tcode{file_status(file_type::fifo)}.
\item Otherwise, if the attributes indicate a socket, as if by POSIX
      \tcode{S_ISSOCK}, returns \tcode{file_status(file_type::socket)}.
\item Otherwise, returns \tcode{file_status(file_type::unknown)}.
\end{itemize}
\end{itemize}
...
\returns Same as status(), above, except
      that if the attributes indicate a symbolic link, as if by POSIX \tcode{S_ISLNK}, returns
...
\pnum
\postcondition For the returned path, \tcode{rp}, \tcode{rp.is_absolute()} is \tcode{true}.
...
For Windows-based operating systems, \tcode{system_complete(p)} has the
  ...
  \tcode{p} and \tcode{base} have the same \tcode{root_name()}.
  ...
  for the \tcode{p.root_name()} drive may have been set by prior program,
...
\begin{note} \Cpp does not define the function \tcode{gets}. \end{note}
...
\pnum
Table~\ref{tab:iostreams.hdr.cinttypes} describes header \tcode{<cinttypes>}. \begin{note} The
