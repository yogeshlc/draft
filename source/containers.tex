...
\tcode{X}~(\ref{tab:containers.node.compat}),
\tcode{u} denotes the name of a variable being declared,
...
\tcode{X::insert_return_type} &
 ...
 public data members:\br
   \tcode{bool inserted;}\br
   \tcode{X::iterator position;}\br
   \tcode{X::node_type node;}\br
 The type shall be \tcode{MoveConstructible}, \tcode{MoveAssignable},
 \tcode{DefaultConstructible}, \tcode{Destructible}, and
...
\tcode{a.extract(q)}              &
 \tcode{node_type}             &
 Removes the element pointed to by to \tcode{q}.
...
\tcode{a_uniq.}\br
 ...
 \tcode{node} has the previous value of \tcode{nh}, and \tcode{position}
 points to an element with a key equivalent to \tcode{nh.key()}. &
 Average case \bigoh{1}, worst case \bigoh{\tcode{a_uniq.size()}}.  \\ \rowsep
%
\tcode{a_eq.}\br
 \tcode{insert(nh)}           &
 \tcode{iterator}   &
...
\tcode{a.merge(a2)}              &
 ...
 Worst case \bigoh{N\tcode{*a.size()+}N}.  \\ \rowsep
%
\tcode{a.erase(k)}
&   \tcode{size_type}
