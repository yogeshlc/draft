...
\pnum
A handler of type
\indextext{array, handler~of}%
\indextext{type!array, handler~of}%
``array of \tcode{T}'' or
\indextext{function, handler~of}%
\indextext{type!function, handler~of}%
``function returning \tcode{T}''
is adjusted to be of type
``pointer to \tcode{T}'' or
``pointer to function returning \tcode{T}'',
respectively.
...
\item%
the \term{handler} is of type \textit{cv} \tcode{T} or
\tcode{const T\&} where \tcode{T} is a pointer type and
\tcode{E} is a pointer type that can be converted to \tcode{T}
by one or more of
\begin{itemize}
