...
if and only if \tcode{hash<remove const t<T>>} is enabled.
When enabled, for an object \tcode{o} of type \tcode{optional<T>},
if \tcode{bool(o) == true}, \tcode{hash<optional<T>>()(o)}
shall evaluate to the same value as \tcode{hash<remove const t<T>>()(*o)};
...
if and only if every specialization in \tcode{hash<remove const t<Types>>...} is enabled.
\end{itemdescr}
