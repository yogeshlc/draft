...
namespace std {
  ...
  // \ref{variant.in_place}, In-place construction
  ...
  // \ref{variant.monostate}, Class monostate
  struct monostate;

  // \ref{variant.monostate.relops}, monostate relational operators
  ...
  // \ref{variant.bad_variant_access}, class bad_variant_access
...
\pnum
All types in \tcode{Types...} shall be (possibly \textit{cv}-qualified)
object types, (possibly \textit{cv}-qualified) \tcode{void}, or references.
\begin{note}
...
This function shall not participate in overload resolution unless
\tcode{is_default_constructible_v<$T_0$>} is true.
...
\remarks
This function shall not participate in overload resolution unless
\tcode{is_copy_constructible_v<$T_i$>} is true for all $i$.
\end{itemdescr}
...
This function shall not participate in overload resolution unless
\tcode{is_move_constructible_v<$T_i$>} is true for all $i$.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
Let \tcode{VS} denote \tcode{variant_size<T>} of the \textit{cv}-unqualified
...
\textit{cv}-unqualified type \tcode{T}. Then each of the three templates shall
...
\rSec2[variant.in_place]{In-place construction}
...
\pnum
\requires
\tcode{I < sizeof...(Types)}, and $T_I$ is not (possibly \textit{cv}-qualified) \tcode{void}.
Otherwise the program is ill-formed.
...
not (possibly \textit{cv}-qualified) \tcode{void}. Otherwise, the program is ill-formed.
...
\pnum
\effects
Equivalent to \tcode{return (v.valueless_by_exception() \&\& w.valueless_by_exception()) || (v.index() == w.index() \&\& get<$i$>(v) == get<$i$>(w))} with $i$ being \tcode{v.index().}
\end{itemdescr}
...
\pnum
\effects
Equivalent to \tcode{return (v.index() < w.index()) || (v.index() == w.index() \&\& !v.val\-ueless_by_exception() \&\& get<$i$>(v) < get<$i$>(w))} with $i$ being \tcode{v.index()}.
\end{itemdescr}
...
\rSec2[variant.bad_variant_access]{Class \tcode{bad_variant_access}}
...
\pnum
\begin{note}
Specialization of this trait informs other library components that variant can be constructed with an allocator, even though it does not have a nested allocator_type.
\end{note}
\end{itemdescr}
