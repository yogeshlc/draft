...
namespace std {
  ...
  // \ref{to.chars}, output functions
  struct to_chars_result {
    char* ptr;
    bool overflow;
  };

  to_chars_result to_chars(char* first, char* last, @\seebelownc@ value, int base = 10);
  ...
  // \ref{from.chars}, input functions
...
\pnum
The type \tcode{chars_format} is a bitmask type~(\ref{bitmask.types}) with elements
\tcode{scientific}, \tcode{fixed}, and \tcode{hex}.

\pnum
For the \tcode{to_chars} function taking a parameter \tcode{base},
the implementation shall provide overloads
for all signed and unsigned integer types and \tcode{char}
...
\rSec2[to.chars]{Output functions}

\pnum
All functions named \tcode{to_chars}
convert \tcode{value} into a character string
by successively filling the range \range{first}{last}.
If the member \tcode{overflow} of the return value is \tcode{false},
the conversion was successful and
the member \tcode{ptr} is the one-past-the-end pointer of the characters written.
Otherwise, the member \tcode{ptr} has the value \tcode{last}
and the contents of the range \range{first}{last} are unspecified.

\indexlibrary{\idxcode{to_chars}|(}%
\begin{itemdecl}
to_chars_result to_chars(char* first, char* last, @\textit{T}@ value, int base = 10);
\end{itemdecl}
...
\pnum
\effects
The value of \tcode{value} is converted to
a string of digits in the given base (with no redundant leading zeroes).
Digits in the range 10...35 (inclusive) are represented as
lowercase characters \tcode{'a'}...\tcode{'z'}.
If value is less than zero, the representation starts with \tcode{'-'}.
...
as-if by \tcode{printf} in the ``C'' locale (see ISO C 7.21.6.1).
If \tcode{hex} is \tcode{true},
the conversion specifier is \tcode{a} (without leading \tcode{"0x"});
otherwise the conversion specifier is \tcode{f} or \tcode{e}.
In either case,
the representation is such that
there is at least one digit before the radix point (if present) and
the representation requires a minimal number of characters,
yet parsing the representation using the corresponding \tcode{from_chars} function
recovers value exactly.
\begin{note}
This guarantee applies only if \tcode{to_chars} and \tcode{from_chars}
...
\effects
\tcode{value} is converted to a string
as-if by printf in the ``C'' locale
with the given precision (see ISO C 7.21.6.1).
...
\tcode{a} (without leading \tcode{"0x"} in the result) if \tcode{fmt} is \tcode{chars_format::hex}, and
...
\rSec2[from.chars]{Input functions}

All functions named \tcode{from_chars}
analyze the string \range{first}{last} for a pattern.
...
are interpreted as a representation of a value of type \tcode{T}.
The member \tcode{ptr} of the return value
points to the first character not matching the pattern,
or has the value \tcode{last} if all characters match.
If the parsed value is not in the range
representable by the type of \tcode{value},
\tcode{value} is unmodified and
the member \tcode{ec} of the return value is equal to \tcode{errc::result_out_of_range}.
Otherwise, \tcode{value} is set to the parsed value and
the member \tcode{ec} is set such that
the conversion to \tcode{bool} yields \tcode{false}.
...
if the value of \tcode{base} is 16, and
except that a minus sign is the only sign that may appear, and
only if \tcode{T} is a signed type.
...
\begin{itemdecl}
from_chars_result from_chars(const char* first, const char* last, float& value, bool hex = false);  
from_chars_result from_chars(const char* first, const char* last, double& value, bool hex = false);  
from_chars_result from_chars(const char* first, const char* last, long double& value, bool hex = false);  
\end{itemdecl}
...
except that the only sign that may appear is a minus sign.
If \tcode{hex} is \tcode{true}, the prefix \tcode{"0x"} shall not appear,
but is assumed.
In any case, the resulting value is one of at most
