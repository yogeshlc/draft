...
\defn{compatible with} a pointer type \tcode{T*} when either
\tcode{Y*} is convertible to \tcode{T*} or
\tcode{Y} is \tcode{U[N]} and \tcode{T} is \tcode{U \textit{cv} []}.
...
\tcode{delete p}, when \tcode{is} not an array type,
...
\pnum\effects When T is not an array type,
...
returns a nonzero value, \tcode{p, shared_ptr<T>(r, p)}.
...
\pnum\returns  \tcode{shared_ptr<T>(r, reinterpret_cast<typename shared_ptr<T>::element_type*>(r.get()))}.
