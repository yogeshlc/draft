...
conflicts with any potentially concurrent access to the same object~(\ref{class.mi}).
...
Enables \tcode{shared_from_this} with the value that was returned by \tcode{r.release()}.
If an exception is thrown, the constructor has no effect.
\end{itemdescr}
